class zcl_wcb_tr_invoice_kunrg definition
  public
  final
  create public .
...
class zcl_wcb_tr_invoice_kunrg implementation.
...
* local data
    ...
    create data rr_source_value type kunrg. "wlf_ref_doc_number.
    assign rr_source_value->* to field-symbol(<rr_source_value>).
* read data
    try.
        select kunrg into @data(ld_kunrg)
        from vbrk up to 1 rows
        where vbeln = @<komlfp>-wbelnv.
        endselect.
        <rr_source_value> = ld_kunrg.
